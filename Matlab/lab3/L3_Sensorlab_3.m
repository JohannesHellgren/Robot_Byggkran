...
while keep
    ...
    Gxf = alpha * DATA(z-1,4) + alpha * (GxNew -  GxOld);
    GxOld = GxNew;
    ...
    angleX = alphaG*(angleX + angularVelocity(1)*dt) + (1-alphaG)*(rollf); 
    angleY = alphaG*(angleY + angularVelocity(2)*dt) + (1-alphaG)*(pitchf); 
    angleZ = alphaG*(angleZ + angularVelocity(3)*dt);
    
    DATA(z,:) = [Axf Ayf Azf Gx Gy Gz T1 T2 pitchf rollf Ap Ar Gxsum angleX angleY angleZ Gxf]; 
    if (z>100),
        ...
        plot(DATA((z-100):z,17),'k'); hold on; % Gxf
        
        plot(DATA((z-100):z,13),'r'); hold on;% Gxsum
        
        plot(DATA((z-100):z,12),'k'); hold on;% Ar
        
        plot(DATA((z-100):z,14),'g'); hold off; % angleX
        %plot(DATA((z-100):z,14),'b'); hold on; % angleY
        %plot(DATA((z-100):z,14),'b'); hold off; % angleZ
        
        legend('Gx - gyro vinkelhastighet','Gxf - filtrerad gyro vinkelhastighet','Gxsum - gyro vingel','Ar - roll','angleX - comp. filter');
        
        ylim([-250 250]);  % Don't forget to change resolution if nesserary
        grid on;
        drawnow();
    end
    z = z + 1;
...
legend('com. filter','LP Acceleration');
